...
public class HTTPConnection implements GlobalConstants, HTTPClientModuleConstants
{
    ...
    private static Vector<String>        non_proxy_addr_list = new Vector<String>();
    private static Vector<String>        non_proxy_mask_list = new Vector<String>();
    ...
    private static SSLSocketFactory defaultSSLFactory = null;
	...
	if (prot == HTTPS  &&  defaultSSLFactory == null)
	...
	{
	    byte[] addr = non_proxy_addr_list.elementAt(idx).getBytes();
	    byte[] mask = non_proxy_mask_list.elementAt(idx).getBytes();
    ...
    /**
     * Get the current SSL socket factory for this connection.
     *
	...
	if ((slash = host.indexOf('/')) != -1)	// IP subnet
	{
	    ...
	    ip_addr_str = host.substring(0, slash);
	    ip_mask_str = host.substring(slash+1);
	...
	non_proxy_addr_list.addElement(ip_addr_str);
	non_proxy_mask_list.addElement(ip_mask_str);
    }
	...
	while (try_count-- > 0)
	{
		    ...
		    (sock = input_demux.getSocket()) == null)
		{
			...
			X509Certificate[] certs = ((SSLSocket) sock).
					getSession().getPeerCertificateChain();
			if (certs == null)	// IBM JDK bug
			    throw new SSLPeerUnverifiedException("peer not authenticated");
			checkCert(certs[0], Host);
		    }
