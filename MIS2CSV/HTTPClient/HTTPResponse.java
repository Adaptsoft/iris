...
public class HTTPResponse implements HTTPClientModuleConstants
{
	...
	/**
	* Returns a LinkedList of all the headers with keys and values.
	*
	* @exception IOException If any exception occurs on the socket.
	* @exception ModuleException if any module encounters an exception.
	*/
	public LinkedList<String[]> getHeaders() throws IOException, ModuleException
	{
		if (!initialized)  handleResponse();

		Enumeration keyEnumeration = Headers.keys();

		LinkedList<String[]> keyValPair = new LinkedList<String[]>();
